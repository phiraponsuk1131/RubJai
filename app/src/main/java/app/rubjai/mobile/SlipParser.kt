...
object SlipParser {
    ...
    fun parse(text: String, source: String): DraftTransaction {
        ...
        val merchant = recognizedMrDiy ?: lines.firstOrNull { line ->
        ...
        val date = datePattern.find(text)?.value ?: thaiSlipDatePattern.find(text)?.groupValues?.getOrNull(1)?.replace(Regex("\\s+"), " ")?.trim().orEmpty()
